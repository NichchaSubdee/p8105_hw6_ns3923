...
cv_results |>
  ...
  col.names = c("Proposed model",
              "Main effect only",
              "2&3-way interactions of head x length x sex"
                ))
```

| Proposed model | Main effect only | 2&3-way interactions of head x length x sex |
|---:|---:|---:|
...
alone does not capture enough variation in birthweight. Similarly, the
interaction model performed better than the main-effect model, but
slightly worse than the proposed model (RMSE = 288.38 grams). In
contrast, the proposed model has the lowest average cross-validated RMSE
with 282.38 grams. This indicates that it provides the most accurate
birthweight predictions among the three models.
